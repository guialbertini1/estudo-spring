...
public class ViltianController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			case "UPDATE":
				...
				break;
			case "DELETE":
				deleteViltian(request, response);
				break;
			default:
				listViltians(request, response);
			}
	...
	private void deleteViltian(HttpServletRequest request, HttpServletResponse response) throws Exception{
		int id = Integer.parseInt(request.getParameter("viltianID"));
		
		viltianDB.deleteViltian(id);
		
		response.sendRedirect("http://localhost:8080/jsp-banco/viltian");
	}
